...
export default function SalesTab() {
  ...
  const loadData = async () => {
    try {
      ...
      const customers = companiesRes.data.filter(c => 
        c.company_type === 'CUSTOMER' || c.company_type === 'BOTH'
      )
  ...
  const handleSave = async () => {
    ...
    const warnings = items.map(item => checkExpiry(item.drug_id)).filter(Boolean)
    if (warnings.length > 0) {
      setExpiryWarnings(warnings)
      if (!confirm('Some items are expiring soon. Continue?')) {
        return
      }
    }
    ...
    try {
      ...
      alert('Error saving sale: ' + (error.response?.data?.detail || error.message))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {expiryWarnings.length > 0 && (
        <div className="bg-yellow-900 border border-yellow-700 rounded p-4 flex items-center gap-2">
          <AlertTriangle className="text-yellow-500" />
          <div>
            <p className="text-yellow-200 font-semibold">Expiry Warnings:</p>
            {expiryWarnings.map((warn, i) => (
              <p key={i} className="text-yellow-300 text-sm">{warn}</p>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-4">
        ...
        >
          <Save size={20} /> {loading ? 'Saving...' : 'Save Sale'}
        </button>
        <button
          onClick={handlePrint}
