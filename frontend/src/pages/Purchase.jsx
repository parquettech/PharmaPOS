...
import { generatePurchaseNumbers } from '../utils/numberGenerator'
...
function Purchase() {
  ...
  // Load suppliers and middlemen
  useEffect(() => {
    loadCompanies()
    loadPurchaseNumbers()
  }, [])

  // Load purchase numbers to generate next sequence
  const loadPurchaseNumbers = async () => {
    try {
      ...
      // Fetch purchases for current year to determine next sequence
      const purchases = await purchaseService.getAllPurchases(yearStart, yearEnd)
      
      // Generate next numbers
      const numbers = generatePurchaseNumbers(purchases)
      ...
      if (!error.isNetworkError || error.originalError?.code !== 'ECONNABORTED') {
        console.error('Error loading purchase numbers:', error)
      }
      const numbers = generatePurchaseNumbers([])
      setInvoiceNo(numbers.invoiceNo)
      setBillNo(numbers.billNo)
      setOrderNo(numbers.orderNo)
    }
  ...
  const loadCompanies = async () => {
    try {
      ...
      setSuppliers(fromOptions)
      setMiddlemen(toOptions)
    } catch (error) {
      console.error('Error loading companies:', error)
      setSuppliers([])
      setMiddlemen([{ id: null, name: 'None' }])
  ...
  const selectedSupplierDetails = suppliers.find(s => String(s.id) === String(selectedSupplier))
  const selectedMiddlemanDetails = selectedMiddleman 
    ? middlemen.find(m => m.id !== null && String(m.id) === String(selectedMiddleman))
    : { name: 'None', address: '' }
  ...
  const handleSavePurchase = async () => {
    ...
    try {
      ...
      const purchaseData = {
        ...
        middleman_id: selectedMiddleman || null,
        invoice_no: invoiceNo || null,
        bill_no: billNo || null,
        purchase_date: date, // Date is already in YYYY-MM-DD format from date input
      ...
      // Trigger stock update event to refresh Stock and StockList pages
      window.dispatchEvent(new CustomEvent('stockUpdated'))
      
      alert('Purchase saved successfully! Stock quantities have been updated.')
      ...
      alert(error.message || 'Failed to save purchase. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  if (isLoading && suppliers.length === 0 && middlemen.length === 0) {
    return (
      <div className="p-3 sm:p-4 md:p-6 bg-slate-900 min-h-screen">
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-4 sm:p-6">
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6">Purchase</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div>
            ...
            <FilterableDropdown
              ...
              onChange={(value) => setSelectedMiddleman(value === null ? null : value)}
              placeholder="Select Company"
              filterPlaceholder="Type to filter companies..."
              className="w-full"
              disabled={isLoading}
            ...
            <div>
              ...
              <div className="px-3 py-2 text-sm bg-slate-700/30 border-2 border-slate-600 rounded-lg text-white min-h-10">
                {selectedSupplierDetails ? (
                  <div>
                    <div className="font-medium">{selectedSupplierDetails.name}</div>
                    {selectedSupplierDetails.address && (
                      <div className="text-xs text-slate-300 mt-1">{selectedSupplierDetails.address}</div>
                    )}
                  </div>
                ) : (
